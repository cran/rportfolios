random.shortonly.test <- function(  n=2, k=n, segments=NULL, x.t=1, 
...
### segments = a vector or list of vectors that define portfolio segments
...
###
    ...
    thisResult <- random.longonly.test( n, k, segments, x.t, x.l, x.u, 
        max.iter )
    x <- -thisResult$x
